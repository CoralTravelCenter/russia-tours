...
ASAP(function() {
  var $flickityReady, $hotels_list, data_grids, expandLinkWithDates, hotels_list, month, parseDataSource, responsiveHandler, tomorrow, wrapTilesBy;
  ...
  expandLinkWithDates = function(href, dates_list) {
    var duration, params_strings, query, query_string, query_url;
  ...
  parseDataSource = function(ugly_cms_tables) {
    return $(ugly_cms_tables).get().map(function(table) {
  ...
  data_grids = parseDataSource('[data-hotels-datasource]');
  data_grids.forEach(function(data_grid, data_grid_idx) {
    var desktop_layout, month_names, renderMobileTable;
    ...
    if (data_grid.length) {
      ...
      desktop_layout = responsiveHandler('(min-width: 769px)', function() {
        ...
        return $('[id=_tours_table]').eq(data_grid_idx).empty().append(Mustache.render($('#_tours_table_desktop_template').html(), {
        ...
        if (opt === 'tour') {
          return $('[id=_tours_table]').eq(data_grid_idx).empty().append(renderMobileTable('tour_uniq_name', 'month_name'));
        } else if (opt === 'month') {
          return $('[id=_tours_table]').eq(data_grid_idx).empty().append(renderMobileTable('month_name', 'tour_uniq_name'));
        }
      });
      return $('[data-term-option]').on('click', function() {
        var $this, opt;
        $this = $(this);
