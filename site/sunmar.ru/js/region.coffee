...
#                by_tour = _.groupBy(data_grid, 'tour_name')
    ...
    if hotels_list.length and $hotels_list.length
        $hotels_list.prepend Mustache.render $('#_hotels_list_template').html(), hotels_list: hotels_list
        $.when($flickityReady).done ->
            ...
            responsiveHandler '(min-width: 769px)',
                ->
                    ...
                    $slider.find('.progress-scrubber').css width: (_.clamp(3 / $slider.data('flickity').cells.length * 100, 100) + '%')
                ->
                    $slider = $('.recommended .hotels-list')
                    $slider.find('.progress-scrubber').css width: (_.clamp(2 / $slider.data('flickity').cells.length * 100, 100) + '%')
            responsiveHandler '(min-width: 577px)',
                ->
                    $slider = $('.recommended .hotels-list')
                    $slider.find('.progress-scrubber').css width: (_.clamp(2 / $slider.data('flickity').cells.length * 100, 100) + '%')
                ->
                    $slider = $('.recommended .hotels-list')
                    $slider.find('.progress-scrubber').css width: (_.clamp(1 / $slider.data('flickity').cells.length * 100, 100) + '%')
    else
